	In this section we show that the Linear Steklov method 
\crefrange{eqn:SSLSM1}{eqn:SSLSM2} converges  with a standard one-half order. For that,
...
to \Cref{ass:OSLC,ass:ajBound,ass:HypThmSingularities} we also require the following.
 ...
 \begin{thm}\label{thm:EulerConvergenceRateHMS}[{\citet[Thm 4.4]{Higham2002b}}]
 	Under \Crefrange{ass:ajBound}{ass:PolynomialGrowth} the EM solution with continuous extension 
...
\begin{proof} Using bound  \eqref{eqn:AfterTriangle} then by  lemma \ref{lem:yhyOh} and since
 ...
 we can use \Cref{thm:EulerConvergenceRateHMS} and conclude that the \SM has order one-half. 
\end{proof}
